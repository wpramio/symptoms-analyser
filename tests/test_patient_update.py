...
def test_db_path(tmp_path):
    ...
    cursor.execute("INSERT INTO patients (id, pseudonym, real_name) VALUES (1, 'Paciente1', 'John Doe')")
    cursor.execute("INSERT INTO patients (id, pseudonym, real_name) VALUES (2, 'Paciente2', 'Jane Doe')")
    cursor.execute("INSERT INTO therapy_session_patients (therapy_session_id, patient_id) VALUES (1, 1)")
    cursor.execute("INSERT INTO patient_item_scores (evaluation_id, patient_id, dimension_code, item_code, score) VALUES (10, 1, '1', '1.1', 3)")
    conn.commit()
    conn.close()
...
def test_update_patient_success_real_name_only(mock_get_db):
    with mock.patch("symptoms_analyser.db.orm.get_db", mock_get_db):
        ...
        with mock_get_db() as conn:
            row = conn.execute("SELECT real_name FROM patients WHERE pseudonym = 'Paciente1'").fetchone()
...
def test_update_patient_success_full(mock_get_db):
    with mock.patch("symptoms_analyser.db.orm.get_db", mock_get_db):
        ...
        with mock_get_db() as conn:
            row = conn.execute("SELECT id, pseudonym, real_name FROM patients WHERE pseudonym = 'Paciente8'").fetchone()
            assert row["id"] == 1
            assert row["pseudonym"] == "Paciente8"
            assert row["real_name"] == "John Legend"
            
            assert conn.execute("SELECT count(*) FROM patients WHERE pseudonym = 'Paciente1'").fetchone()[0] == 0
            
            # Since the primary key ID is stable (value 1), child records stay perfectly intact
            assert conn.execute("SELECT count(*) FROM therapy_session_patients WHERE patient_id = 1").fetchone()[0] == 1
            assert conn.execute("SELECT count(*) FROM patient_item_scores WHERE patient_id = 1").fetchone()[0] == 1
