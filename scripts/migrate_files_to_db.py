    ...
    for run in runs:
        ...
        if cursor.fetchone() is None:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, 'txt', 'Transcript raw text missing in migration', 'completed', 100.0)
            """, (session_name, f"{session_name}.txt"))
...
def ingest_clinical_evaluations(conn):
    ...
    for run in runs:
        ...
        if cursor.fetchone() is None:
            ...
            cursor.execute("""
                INSERT INTO transcripts (id, filename, file_type, raw_text, status, progress_percent)
                VALUES (?, ?, 'txt', 'Autogenerated transcript during scoring ingestion', 'completed', 100.0)
            """, (clean_transcript_id, f"{clean_transcript_id}.txt"))
