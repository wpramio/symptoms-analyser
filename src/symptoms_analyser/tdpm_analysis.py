...
def write_log(output_dir: Path, run_entry: dict, output_data: dict = None, db_conn: sqlite3.Connection = None, evaluator_id: str = "clinician_1"):
    # Note: cumulative log writing (tdpm_analysis.log.json) has been deprecated to optimize database integration.
    ...
    if output_data:
        try:
            conn = db_conn
            should_close = False
            if conn is None and db_path.parent.exists():
...
def main():
    ...
    try:
        # Step 1 — Load text (from DB or physical file)
        if args.transcript_id:
            ...
            if not row or not row[1]:
                print(f"Error: Transcript ID '{session_name}' not found or does not have sanitized text in database.", file=sys.stderr)
                sys.exit(1)
            
            # Mock input_path for downstream output compatibility
            ...
            # Extract session name (e.g. Paciente1.run1.sanitized -> Paciente1)
            # Remove .runX.sanitized and .raw and other suffixes
        ...
        if args.output:
            if args.output.is_dir() or args.output.suffix == "":
                ...
                output_path = args.output / f"{session_name}.{timestamp}.tdpm.json"
            else:
        ...
        if output_path:
            ...
            # Write to SQLite via write_log
            log_entry = {
                "timestamp_utc": datetime.now(timezone.utc).isoformat(),
                "session": session_name,
                "input_file": str(input_path.resolve()) if not args.transcript_id else session_name,
            ...
            write_log(output_path.parent, log_entry, output_data=output, db_conn=db_conn, evaluator_id=args.evaluator_id)
        else:
